...
实验链接（https://pdos.csail.mit.edu/6.S081/2020/labs/syscall.html ）  
...
第1种的想法和下面注释写的一样：不知道怎么处理的情况下，就逐渐增加内存，直到不能再增加；结束后别忘了释放内存（因为当时大概看过用户程序通过“**sbrk**”函数增加内存，而“**sbrk**”函数在内核里面实际上最终使用了“**growproc()**”函数）
```
